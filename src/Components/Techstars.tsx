    ...
    <div className="bg-[#ccdeeb] text-[#00121f] py-10 px-4">
      ...
      <div className="flex items-center justify-center flex-wrap gap-6">
        {Array(4).fill(null).map((_, index) => (
          <div key={index} className="flex items-center space-x-2">
            ...
            <Image src="/logo.png" alt="Techstars Logo" width={60} height={60} />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 mt-10">
        {features.map((feature, index) => (
          <div
            key={index}
            className="bg-white rounded-2xl shadow-md p-6 hover:shadow-lg transition-shadow duration-300 relative"
          >
            <GoArrowUpRight className="absolute top-4 left-4 text-[#00121f] text-2xl border border-[#f0f0f0] shadow-2xl rounded-xl p-1 bg-[#ccdeeb]" />
